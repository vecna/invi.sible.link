        ...
        return jsonReader(path.join(configContent.world.locationDir, tlc + '.json'))
            .then(function(countryURLsContent) {
                return _.map(countryURLsContent, function(siteEntry) {
                    ...
                    /* here sompe computation are done, but not the Impact assessment, that's
                       is part of lib/analytics and plugin/ranks */
                    return {
                        country: tlc,
                        href: siteEntry.href,
                        ...
                        reachPM: reachPM,
                        nationalPrcnt: _.round( ((1000 * 1000)/reachPM), 2) * 100,
