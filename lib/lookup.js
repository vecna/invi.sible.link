...
var byCategory = function(si, catName) {
    return _.reduce(si.world, function(memo, crawledEntry) {
        if(!(_.isUndefined(crawledEntry.categories))) {
            _.each(crawledEntry.categories, function(cat) {
                if(cat.where === catName)
                    memo.push(crawledEntry);
            });
        }
        return memo;
    }, []);
