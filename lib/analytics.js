...
var reputDot = function(str) { return _replace(str, 'ł', '.'); };
var awayDot = function(str) { return _replace(str, '.', 'ł'); };
...
/* use visibility multiplied per Companies */
    ...
    console.log(JSON.stringify(retMap, undefined, 2));

    _.each(pl.data, function(siteTested) {
    /* REMIND: if siteTested.log is {}, then OFC can't work */
        var id = _.trunc(siteTested.log.hash, { length: 6, omission: ''});
        _.each(siteTested.rr, function(inclusion) {
            if (_.isUndefined(retMap[id])) {
                console.log(JSON.stringify(siteTested, undefined, 2));
                debugger;
            }
            if (!_.isUndefined(inclusion.company) &&
