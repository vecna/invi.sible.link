...
function tableReduction(coll) {
    var kept = [ 'url', 'domaindottld', 'needName', 'VP', 'unrecognized', 'when', 'cookies', 'leaders' ];
    if(!_.size(coll))
        return [];
    var ret = _.reduce(coll, function(memo, e) {
        memo.push(_.pick(e, kept));
        return memo;
    }, []);
...
function getEvidences(filter, past) {

    var last = moment().subtract(past, 'h').format("YYYY-MM-DD");
    filter.when = new Date(last);
    debug("getEvidences with filter %j", filter);

    return mongo
        .read(nconf.get('schema').evidences, filter);
};

function smallEvidences(entry) {
    var fields = [ 'url', 'cookies', 'company',
                   'companyC', 'domaindottld',
