    ...
    return fs
        ...
        .then(function(jsonString) {
            ...
            if (!_.endsWith(sourceFile, '.log')) {
                ...
                debug("jsonReader Log from %s (%d) of %s",
                    parsed.completed, (parsed.endLoad - parsed.startLoad), debugName);
            }
            return parsed;
        })
        .catch(function(e) {
...
var usedLocation = function()
{
    ...
    if(process.env.FROMLIST_DETAILS) {
      ...
      whenWant = moment(whenWant-(process.env.DISKCHECK_DAY*24*3600*1000)).format('YYMMDD');
