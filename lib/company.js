...
function initialize() {
    ...
    companyMap = _.reduce(companySource, function(memo, listOfTrack, companyName) {
        _.each(listOfTrack, function(trackerDomain) {
            ...
            debug("found %j from %s", trackerStruct, trackerDomain);
            memo.push(trackerStruct)
        });
        return memo;
    }, []);
    debug("Entry %d", _.size(companyMap));
    return companyMap;
};
...
function attribution(data) {
    ...
    data.data = _.map(data.data, function(inclusion) {
        ...
        if(C) {
            ...
            debug("Company Found! %s %s %s %s in %s", C.subdomain, C.domain, C.tld, C.domaindottld, inclusion.domaindottld);
        }
        return inclusion;
    });
    return data;
