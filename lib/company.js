...
var companySource = {
 ...
 "M\u00e9diam\u00e9trie-eStat": ["mediametrie-estat.com", "estat.com"],
 "NDN": ["newsinc.com"],
    ...
    data.data = _.map(data.data, function(inclusion) {
        if(inclusion.company) {
            if(countryAttr[inclusion.company]) {
                data.companyC = countryAttr[inclusion.company];
                _.set(infos.ok, data.domaindottld);
            }
            else {
                _.set(infos.miss, data.domaindottld);
            }
        }
    });
    debug("countryattr %s [%d ³rd] %d OK %d missing",
        data.page, _.size(data.data),
        _.size(_.keys(infos.ok)),
        _.size(_.keys(infos.miss)) );
    return data;
};
