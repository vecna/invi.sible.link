...
function core(selector, vp, value) {
    return mongo
        ...
        .then(function(p) {
            ...
            _.set(p, vp, value);
            return mongo.updateOne(nconf.get('schema').promises, 
                { _id: p._id }, p)
            .then(function(r) {
                if(r.modifiedCount !== 1)
                    debug("Error: expected modifiedCount === 1 [%j], matches %d", r, _.size(p));
                return p;
            });
        });
}
...
/*
function forceFix(prom) {
    return mongo
        .read(nconf.get('schema').promises, {id: prom.id})
        .then(function(maybetwo) {
