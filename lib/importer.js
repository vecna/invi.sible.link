...
var pickContent = function(headers, phantEstCT) {
    ...
    if(_.startsWith(retVal, '$')) {
        var ctFixer = "something/not-standard";
        debug("Strange content-type %s recorded: %s", retVal, ctFixer);
        retVal = ctFixer;
    }
    return retVal;
};
    ...
    return jsonReader(fileEntry.logFile, fileEntry.debugName)
        ...
        .tap(function() {
            if(isPercentage(cnt, total, 10))
                debug('Log Import: %s %d/%d (%d%%)', 
                    fileEntry.logFile, cnt, total, 
                    _.round(((cnt/total)*100), 1));
        });
};
...
var importPhantput = function(fileEntry, cnt, total) {
    return jsonReader(fileEntry.phantomFile, fileEntry.debugName)
        ...
        .tap(function() {
            if(isPercentage(cnt, total, 10))
                debug('Phantom Import: %s %d/%d (%d%%)',
                    fileEntry.phantomFile,
