...
var importLog = function(fileEntry) {
    var execlogF = fileEntry.file.substr(0, fileEntry.file.length - 11) + 'executions.log';
    return jsonReader(fileEntry.file)
            .then(function(execContent) {
                fileEntry.log = execContent;
                return fileEntry;
            });
};
...
var importPhantput = function(fileEntry) {
    return jsonReader(fileEntry.file)
        .then(function(contentIOList) {
            fileEntry.rr = requestResponseMap(contentIOList);
            return fileEntry;
        });
};
